...
Backend.prototype.init = function() {
  ...
  try {
	...
	mc.connect(this.config.url, this.config, function(err, db){
      if (err) {
        this.log.trace(err);
        return err;
      }
...
Backend.prototype.insert = function(type, user, collection, data, callback) {
  ...
  this.collectionCursor(dbCollectionName, function(cursorException, collectionInstance){
    if (cursorException) {
      callback(cursorException);
      this.log.error(cursorException);
      return null;
    }

    collectionInstance.insert(data, function(insertException, result) {
      if (insertException) {
        this.log.error(insertException);
        return null;
      }
      callback(result._id);
      return null;
...
Backend.prototype.webcollections = function(user, callback) {
  this.collectionCursor("webcollections", function(cursorException, collection) {
    if (cursorException) {
      return callback([]);
    }
    collection.find({owner: user}).toArray(function(toArrayException, result) {
      if (toArrayException) {
        return callback([]);
      }
